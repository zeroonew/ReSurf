...
def resolve_model_dir_and_ext(dense_folder):
    candidate_dirs = [
        os.path.join(dense_folder, "sparse", "0"),
        os.path.join(dense_folder, "sparse"),
    ]
    candidate_exts = [".txt", ".bin"]

    for model_dir in candidate_dirs:
        for ext in candidate_exts:
            cameras_path = os.path.join(model_dir, "cameras" + ext)
            images_path = os.path.join(model_dir, "images" + ext)
            points_path = os.path.join(model_dir, "points3D" + ext)
            if os.path.isfile(cameras_path) and os.path.isfile(images_path) and os.path.isfile(points_path):
                return model_dir, ext

    searched = []
    for model_dir in candidate_dirs:
        for ext in candidate_exts:
            searched.append(os.path.join(model_dir, "cameras" + ext))
            searched.append(os.path.join(model_dir, "images" + ext))
            searched.append(os.path.join(model_dir, "points3D" + ext))
    raise FileNotFoundError(
        "Could not find a COLMAP sparse model under dense_folder.\n"
        "Expected one of sparse/0 or sparse with .txt or .bin files.\n"
        + "\n".join(searched)
    )


def resolve_image_dir(dense_folder):
    candidate_dirs = [
        os.path.join(dense_folder, "images"),
        os.path.join(dense_folder, "mvs", "images"),
    ]
    ...
    raise FileNotFoundError(
        "Could not find an image directory under dense_folder.\n"
        "Expected one of:\n" + "\n".join(candidate_dirs)
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pair_path', type=str, default='', help='Optional output path for pair.txt.')
    ...
    image_dir = resolve_image_dir(args.dense_folder)
    model_dir, model_ext = resolve_model_dir_and_ext(args.dense_folder)
    cam_dir = os.path.join(args.dense_folder, 'cams')
    renamed_dir = os.path.join(args.dense_folder, 'images_vis')
    pair_path = args.pair_path if args.pair_path else os.path.join(args.dense_folder, 'pair.txt')

    cameras, images, points3d = read_model(model_dir, model_ext)
    ...
    os.makedirs(os.path.dirname(os.path.abspath(pair_path)), exist_ok=True)
    with open(pair_path, 'w') as f:
        f.write('%d\n' % len(images))
        for i, sorted_score in enumerate(view_sel):
            # f.write('%s\n%d ' % (images[i+1].name.replace('.png', ''), len(sorted_score)-1))
            f.write('%s\n%d ' % (os.path.splitext(images[i+1].name)[0], len(sorted_score)-1))
            for image_id, s in sorted_score:
                if i == image_id:
                    continue
                # f.write('%s %f ' % (images[image_id+1].name.replace('.png', ''), s))
                f.write('%s %f ' % (os.path.splitext(images[image_id+1].name)[0], s))
            f.write('\n')
    print(f"pair.txt written to {pair_path}")

    if args.convert_format and not args.test:
        os.makedirs(renamed_dir, exist_ok=True)
        for i in range(num_images):
            src_image = os.path.join(image_dir, images[i+1].name)
            img = cv2.imread(src_image)
            if img is None:
                raise FileNotFoundError(f"Failed to read image for conversion: {src_image}")
            cv2.imwrite(os.path.join(renamed_dir, '%08d.jpg' % i), img)
        print(f"Converted images written to {renamed_dir}")
    # for i in range(num_images):
