...
def select_train_images(images, n_views):
    ...
    selected_indices = set(DTU_TRAIN_INDICES[:n_views])
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create DTU sparse-view subsets and dense COLMAP reconstructions for mesh evaluation."
    )
    parser.add_argument("--base_path", required=True, help="Dataset root containing scanXX folders.")
    parser.add_argument("--scenes", nargs="+", default=DEFAULT_SCENES, help="Scene names under base_path.")
    parser.add_argument(
        "--n_views",
        type=int,
        default=3,
        help="Number of sparse training views to keep. Non-positive values keep all available views.",
    )
